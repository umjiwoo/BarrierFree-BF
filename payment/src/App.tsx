...
const App = () => {
  ...
  useEffect(() => {
    ...
    const socket = new WebSocket(`ws://j12a208.p.ssafy.io:8080/api/ws/request-payment`);
    ...
    socket.onmessage = (event) => {
      ...
      if (message.result?.code === 200 && message.result?.message === 'success') {
        setStatus('approved');
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await fetch('http://j12a208.p.ssafy.io:8080/api/transactions/request-payment', {
      method: 'POST',
